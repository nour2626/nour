...
include "C:/xampp/htdocs/projetweb2025/config.php";

$categoriesController = new categoriesController();
$categoriesList = $categoriesController->categoriesList(); // Fetch all categories
...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    if (isset($_POST["category_id"]) && !empty($_POST["category_id"])) {
        ...
        $categoryToDelete = $categoriesController->getcategoriesById($categoryId);

        // Check if the category has associated sites
        $conn = config::getConnexion();
        $stmt = $conn->prepare("SELECT COUNT(*) as site_count FROM sites WHERE category = :categoryId");
        $stmt->execute(['categoryId' => $categoryId]);
        $result = $stmt->fetch();
        $associatedSitesCount = $result['site_count'];

        if ($associatedSitesCount > 0 && !isset($_POST['confirm_delete'])) {
            // If there are associated sites, show a confirmation message
            $confirmationRequired = true;
        } elseif (isset($_POST['confirm_delete']) && $_POST['confirm_delete'] === 'yes') {
            // If user confirmed, delete the category and its sites
            $stmt = $conn->prepare("DELETE FROM sites WHERE category = :categoryId");
            $stmt->execute(['categoryId' => $categoryId]);
                                ...
                                <div class="card-body">
                                    ...
                                    <?php if ($confirmationRequired): ?>
                                        <div class="alert alert-warning">
                                            <p>
                                                The category <strong><?= htmlspecialchars($categoryToDelete['nom_categorie']); ?></strong>
                                                has <strong><?= $associatedSitesCount; ?></strong> associated site(s).
                                                If you delete this category, all associated sites will also be deleted.
                                            </p>
                                            <form method="POST" action="">
                                                <input type="hidden" name="category_id" value="<?= htmlspecialchars($categoryToDelete['id']); ?>">
                                                <input type="hidden" name="confirm_delete" value="yes">
                                                <button type="submit" class="btn btn-danger">Yes, Delete All</button>
                                                <a href="deletecategories.php" class="btn btn-secondary">Cancel</a>
                                            </form>
                                        </div>
                                    <?php elseif ($error): ?>
